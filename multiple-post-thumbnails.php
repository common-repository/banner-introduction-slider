...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function __construct($args = array()) {
			$this->rsfs_register($args);
		}

		/**
		 * rsfs_register a new post thumbnail.
		 *
		 ...
		 *
		 * post_type - The post type to rsfs_register this thumbnail for. Defaults to post.
		 *
		 ...
		 * @return void
		 */
		public function rsfs_register($args = array()) {
			global $wp_version;
			...
			add_action('add_meta_boxes', array($this, 'rsfs_add_metabox'));
			if (version_compare($wp_version, '3.5', '<')) {				
				add_filter('attachment_fields_to_edit', array($this, 'rsfs_add_attachment_field'), 20, 2);
			}
			add_action('admin_enqueue_scripts', array($this, 'rsfs_enqueue_admin_scripts'));
			add_action('admin_print_scripts-post.php', array($this, 'rsfs_admin_header_scripts'));
			add_action('admin_print_scripts-post-new.php', array($this, 'rsfs_admin_header_scripts'));
			add_action("wp_ajax_set-{$this->post_type}-{$this->id}-thumbnail", array($this, 'rsfs_set_thumbnail'));
			add_action('delete_attachment', array($this, 'rsfs_action_delete_attachment'));
			add_filter('is_protected_meta', array($this, 'rsfs_filter_is_protected_meta'), 20, 2);
		}
		 ...
		 * @return string 
		 */
		public function rsfs_get_meta_key() {
		 ...
		 * @return void
		 */
		public function rsfs_add_metabox() {
			add_meta_box("{$this->post_type}-{$this->id}", __($this->label, 'multiple-post-thumbnails'), array($this, 'rsfs_thumbnail_meta_box'), $this->post_type, $this->context, $this->priority);
		}
		 ...
		 */
		public function rsfs_thumbnail_meta_box() {
			global $post;
			
			$thumbnail_id = get_post_meta($post->ID, $this->rsfs_get_meta_key(), true);
		 ...
		 * @return void
		 */
		public function rsfs_add_attachment_field($form_fields, $post) {
			$calling_post_id = 0;
		 ...
		 * @return void
		 */
		public function rsfs_enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {	
				add_thickbox();
				wp_enqueue_script( "mpt-featured-image", $this->rsfs_plugins_url( 'js/multi-post-thumbnails-admin.js', __FILE__ ), array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "mpt-featured-image", $this->rsfs_plugins_url( 'js/multi-post-thumbnails-admin.js', __FILE__ ), array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "mpt-featured-image-modal", $this->rsfs_plugins_url( 'js/media-modal.js', __FILE__ ), array( 'jquery', 'media-models' ) );				
			}
			
			wp_enqueue_style( "mpt-admin-css", $this->rsfs_plugins_url( 'css/multi-post-thumbnails-admin.css', __FILE__ ) );
		}
		
		public function rsfs_admin_header_scripts() {
			$post_id = get_the_ID();
		 ...
		 * @param int $post_id
		 */
		public function rsfs_action_delete_attachment($post_id) {
			global $wpdb;

			$wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE meta_key = '%s' AND meta_value = %d", $this->rsfs_get_meta_key(), $post_id ));
		}
		 ...
		 * @return boolean 
		 */
		public function rsfs_filter_is_protected_meta($protected, $meta_key) {
			if (apply_filters('mpt_unprotect_meta', false)) {
				return $protected;
			}
			
			if ($meta_key == $this->rsfs_get_meta_key()) {
				$protected = true;
			}
		 ...
		 */
		private function rsfs_plugins_url($relative_path, $plugin_path) {
		 ...
		 * @param string $id The id used to rsfs_register the thumbnail.
		 ...
		 *
		 * @param string $post_type The post type.
		 * @param string $thumb_id The id used to rsfs_register the thumbnail.
		 * @param string $post_id Optional. Post ID.
		 ...
		 */
		public function rsfs_set_thumbnail() {
			...
			if ($thumbnail_id == '-1') {
				delete_post_meta($post_ID, $this->rsfs_get_meta_key());
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					$this->rsfs_set_meta($post_ID, $this->post_type, $this->id, $thumbnail_id);
		 ...
		 * @param string $thumbnail_id ID used to rsfs_register the thumbnail
		 * @param int $thumbnail_post_id ID of the attachment to use as the thumbnail
		 * @return bool result of update_post_meta
		 */
		public static function rsfs_set_meta($post_ID, $post_type, $thumbnail_id, $thumbnail_post_id) {
